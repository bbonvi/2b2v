...
const SEED_SUBJECTS: Record<(typeof PRIVATE_LIFE_TERRITORIES)[number], readonly string[]> = {
  ...
  "computing-systems": [
    "an undocumented chip on an old board",
    "an unstable power rail",
    "an unusual boot sequence",
    "an unknown connector or pinout",
    "a microcontroller peripheral",
    "a hardware and firmware mismatch",
    "an obsolete computer architecture",
    "a strange memory timing problem",
    "a storage controller behaving incorrectly",
    "an intermittent solder joint",
    "a poor PCB layout",
    "a signal integrity problem",
    "an oscilloscope trace that makes no sense",
    "an obsolete bus or protocol",
    "unexpected traffic from a quiet device",
    "a small network service with an odd failure",
    "a radio signal with a repeating pattern",
    "an antenna that works for the wrong reason",
    "a device with firmware worth disassembling",
    "a display or audio device worth repurposing",
    "a sensor interface with too much noise",
    "a driver that disagrees with the hardware",
    "a tool she could build instead of buying",
    "a technically useless experiment",
  ],
