...
describe("createMemoryListTool", () => {
  ...
  test("retrieves user memories available in the selected guild", async () => {
    ...
    createMemory(db, { guildId: "g2", aboutUserId: "u1", kind: "fact", content: "Other guild fact" });
    ...
    expect(text).toContain("User memories for @Alice (user:u1) (2/2 shown):");
  ...
  test("limits returned memories", async () => {
    createMemory(db, { guildId: "g1", aboutUserId: "u1", kind: "fact", content: "First memory" });
    createMemory(db, { guildId: "g1", aboutUserId: "u1", kind: "fact", content: "Second memory" });
    ...
    expect(text).toContain("User memories for @alice (user:u1) (1/2 shown):");
    expect(result.details).toEqual({ target: "user", userId: "u1", count: 1, total: 2 });
  });

  test("defaults returned memories to 30", async () => {
    for (let i = 1; i <= 31; i += 1) {
      createMemory(db, { guildId: "g1", aboutUserId: "u1", kind: "fact", content: `Memory ${i}` });
    }
    ...
    expect(text).toContain("User memories for @alice (user:u1) (30/31 shown):");
  ...
  test("reports no user memories without returning community memories", async () => {
    createMemory(db, { guildId: "g1", kind: "note", content: "Global note" });
    ...
    expect(text).toContain("No user memories available here for @alice (user:u1)");
    expect(text).not.toContain("Global note");
  ...
  test("shows raw user id only in the user memories header", async () => {
    createMemory(db, { guildId: "g1", aboutUserId: "u1", kind: "fact", content: "Portable user note" });
    ...
    expect(text).toContain("User memories for user:u1 (1/1 shown):");
    expect(text).toContain("- 1 [0.7] [fact] Portable user note");
    expect(text).not.toContain("[user:u1]");
    expect(result.details).toEqual({ target: "user", userId: "u1", count: 1, total: 1 });
  });

  test("retrieves community memories by guild id", async () => {
    createMemory(db, { guildId: "g1", kind: "note", content: "Current guild note" });
    createMemory(db, { guildId: "g2", kind: "note", content: "Other guild note" });
    createMemory(db, { guildId: "g2", aboutUserId: "u1", kind: "fact", content: "Portable user note" });
    ...
    const result = await tool.execute("tc1", { target: "community", guild_id: "g2" }, AbortSignal.timeout(5000));
    const text = textOf(result);

    expect(text).toContain("Community memories for Guild Two (g2) (1/1 shown):");
    ...
    expect(result.details).toEqual({ target: "community", guildId: "g2", count: 1, total: 1 });
  });

  test("retrieves self memories", async () => {
    createMemory(db, { guildId: "g1", about: "self", kind: "journal", content: "Privately decided the room matters." });
    createMemory(db, { guildId: "g1", aboutUserId: "u1", kind: "fact", content: "Portable user note" });
    createMemory(db, { guildId: "g1", kind: "note", content: "Guild note" });
    ...
    expect(text).toContain("[about:self] [in:anywhere] [when:always] [0.7] [journal] Privately decided the room matters.");
    expect(text).not.toContain("Portable user note");
    expect(text).not.toContain("Guild note");
    expect(result.details).toEqual({ target: "self", count: 1, total: 1 });
  });

  test("rejects inaccessible community memory reads", async () => {
    createMemory(db, { guildId: "g2", kind: "note", content: "Other guild note" });
  ...
  test("rejects raw user id reads when the user is not in the selected guild", async () => {
    createMemory(db, { guildId: "g1", aboutUserId: "u2", kind: "fact", content: "Portable user note" });
