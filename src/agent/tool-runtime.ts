...
export function createToolRuntime(input: {
    ...
    createBotDiscordMessageSender: ReturnType<typeof import("./turn-runtime").createTurnRuntime>["createBotDiscordMessageSender"];
  ...
  const { db, client, log, agentJobs, scheduler, linkContentCache, getGlobalConfig, getPromptBundle, getGuildConfig, getRelationshipConfig, notebooksEnabled, runtimeToolDescription, runtimeContextTemplate, resolveClientGuild, fetchAccessibleGuildChannel, resolveGuildUsername, resolveGuildMemberReference, fetchEmojiCache, buildOutboundResolvers, createBotDiscordMessageSender, runImageGenerationJob, trackImageJob, watchMatcher, getEventWatchRuntime, getEventWatchDiscordAdapters, emojiCache, innerThreadsEnabled, refreshEmojiCache, loadExternalReference, loadGuildAvatarReference, getVoiceRuntime } = input;
...
function buildAgentTools(
  ...
  if (getGlobalConfig().braveApiKey !== undefined && getGlobalConfig().braveApiKey !== "") {
    tools.push(createBraveSearchTool({ apiKey: getGlobalConfig().braveApiKey }));
    tools.push(createBraveImageSearchTool({ apiKey: getGlobalConfig().braveApiKey }));
  }
