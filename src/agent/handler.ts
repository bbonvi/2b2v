...
function buildRuntimeInstruction(): string {
  return [
    ...
    "If the current message is an internal [Async Image Job Failed] event, the image was not generated. Reply naturally in character. Prefer replying to the original request using the concrete MsgID from the suggested <message reply=\"true\" reply_to=\"123...\">...</message> envelope, but you may reply to another message if the current chat context makes that clearly better. You may mention that the image failed or timed out, but do not paste raw JSON, stack traces, or long internal errors unless the user explicitly asks for technical details. Do not call codex_generate_image, cancel_agent_job, or start another image job from an async failure notification.",
