...
import { isSendableGuildChannel, type SendableGuildChannel } from "../discord/message-sender";
import { handleMessage } from "../agent/handler";
import { runSilentToolAgentPass } from "../agent/maintenance-pass";
import { type IncomingMessage } from "../agent/turn-types";
import { type AssembledContext } from "../agent/context-assembly";
import { type HistoryMessage } from "../agent/history-types";
import { insertPromptOnlyMessageHandoff, upsertBotMessageContent } from "../db/message-state-repository";
import { buildMemoryContext } from "../agent/memory-context";
import { createRecordMemoryTool } from "../agent/memory-extraction";
import { buildInnerThreadsContext, createRecordInnerThreadsTool } from "../agent/inner-thread-service";
...
import { createRecordRelationshipTool, getRelationshipProfile, renderRelationshipAxisValues, renderRelationshipPromptContext, type RelationshipContextProfile, type RelationshipConfig } from "../relationships";
...
import { VoiceRepository } from "../voice/repository.ts";
...
export function createVoiceApplication(input: {
    ...
    defaultPersonaModeForMaintenance: () => { id: string; instructions: string };
  }
) {
  const { db, client, log, requestLogStore, getGlobalConfig, getPromptBundle, getGuildConfig, runtimeContextTemplate, buildContext, buildAgentTools, createHandlerDeps, resolveKnownUsername, resolvePromptUsername, resolveClientGuild, getRelationshipConfig, innerThreadsEnabled, runtimeToolDescription, semanticMaintenanceCoordinator, defaultPersonaModeForMaintenance } = input;
