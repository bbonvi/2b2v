...
Codex Responses reasoning relationships is internal reply-loop state. Assistant turns from the Codex adapter may carry provider-native thinking blocks with encrypted reasoning signatures; keep those blocks with the matching native tool call IDs until the next Codex request after tool results, but never render them into Discord-visible output or persisted prompt-history text. The optional cross-run continuation store may persist only the main visible reply loop's final opaque Codex native content for the same guild/channel/user/model/session until its TTL; silent memory and relationship maintenance continuations must not be saved into that store.
...
Ambient attention is deliberately outside the dispatcher trigger queue. Non-triggered messages may schedule delayed candidates, but each candidate must reload current SQLite history before evaluator spend and again immediately before visible send. Ambient pickup and lingering attention debounce same-user bursts into one pending candidate, pushed by later messages and typing; lingering is additionally scoped to an active attention lease and uses its own shorter typing gate plus higher budget. Ambient runs disable streamed output so the pre-send gate can drop stale replies after generation. Ambient pickup and follow-up default to normal channel messages; lingering attention may default to a Discord reply when anchoring remains useful.

Ambient initiative is separate from message-triggered attention. A jittered per-guild opportunity loop resolves one main channel, builds compact room/presence/open-loop signals, applies shared proactive-speech gates, rolls per-kind pressure, then asks a small evaluator only for whether/kind/target/source/avoidances. The normal reply loop generates the final proactive burst with initiative generation prompts and may still `<ignore>`; pre-send gates run immediately before visible output. Dashboard Prompt Lab can force each initiative kind in draft mode and should show gates, pressure, evaluator, generation, and pre-send results as request-log runtime actions.
...
`promptTransport` controls provider-specific role/target placement for logical sections: `core`, `skills`, `runtime`, `stableContext`, `olderHistory`, `serverMembers`, `threadsInChannel`, `discordContext`, `upcomingSchedules`, `memories`, `recentHistory`, `currentContext`, `responseInstruction`, `currentTurn`, and `finalActionInstruction`. Defaults put core/skills/runtime/responseInstruction in the `developer` role and context/history/current-turn/final-action data in the `user` role. Codex supports `mode: split-input` and `mode: legacy-instructions`; `split-input` keeps stable groups separate in the raw Codex payload, while `legacy-instructions` joins stable text into one instruction string. Volatile input messages are ordered as Discord context, upcoming schedules, memory, recent chat history, server members, threads, current context, response instruction, the current turn, then the final action instruction. Keep transport config deterministic and avoid placing volatile sections before stable cached sections.

Stable persona/style prompt content is loaded from `prompts/core/**/*.md` in deterministic recursive path order. Use numeric file or directory prefixes when order matters, and keep file names/content stable because this text lives in the cached prompt prefix. Runtime instructions are loaded separately from `prompts/runtime/`: normal reply runtime files come from `reply/**`, final per-turn action text from `final-action-instruction/**`, memory context snippets from `memory/context/**`, tool descriptions from `tools/*.md`, tool parameter descriptions from `tool-parameters/<tool>/<param>.md`, volatile context templates from `context/*.md`, fallback vision instructions from `image-reading/fallback-system/**`, ambient evaluator policy from `ambient-attention/evaluator/{shared,ambient-pickup,lingering-attention,follow-up}/**`, and ambient initiative policy from `ambient-initiative/{evaluator,generation}/{shared,self-expression,targeted-checkin}/**`. On-demand skills are manifest-backed directories under `prompts/skills/<id>/`; the stable prompt includes only the generated skill index, while loaded skill bodies enter as `load_skill` tool results. Do not let runtime files leak into the core persona/style prompt bundle.
...
LLM requests include a deterministic `session_id` scoped to guild, channel, provider, and model so provider sticky routing can keep the same cache-warmed endpoint across tool turns and later reply loops. OpenRouter request logs preserve `prompt_tokens_details.cached_tokens` and `cache_write_tokens` for verification. Codex text requests use global `codexTransport`, defaulting to `websocket-cached`; use `sse` only for debugging or transport fallback.
