...
Memory is direct SQLite data by default. Guild memories have `subject_user_id IS NULL` and stay scoped to one guild; user memories have `subject_user_id` set, `guild_id NULL`, and follow the Discord user across guilds. The prompt gets active current-guild memories plus active current-user memories, with temporary expiries rendered relatively. If a user-specific fact only applies in one guild/channel, memory text must say that explicitly. Memory kinds are constrained to notes, preferences, relationships, facts, identity, constraints, interests, and scratchpad. Scratchpad is short-lived internal working context and must always have an expiry. Other users' stored context is signaled through member memory counts, and the read-only `list_memories` tool can retrieve portable user memories or guild memories, including another guild by ID when context requires it.
