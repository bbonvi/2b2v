...
Ambient attention is deliberately outside the dispatcher trigger queue. Non-triggered messages may schedule delayed candidates, but each candidate must reload current SQLite history before evaluator spend and again immediately before visible send. Ambient runs disable streamed output so the pre-send gate can drop stale replies after generation. Ambient pickup and follow-up default to normal channel messages; lingering attention may default to a Discord reply when anchoring remains useful.
