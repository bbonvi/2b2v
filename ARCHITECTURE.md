...
Prompt caching depends on stable content staying stable. OpenRouter receives stable prompt groups before a tiny stable user/assistant anchor and volatile turn context. Keep the group order as core persona/style, compact skills index plus runtime, stable optional context, then older cached history. The Codex subscription backend receives the same stable sequence as the pi-ai system prompt because the Codex Responses adapter owns payload construction.
