...
Any read-only tool may be used when it would materially reduce uncertainty. `record_memory`, `record_relationship`, and `record_inner_threads` are the only state-changing tools available. Call each useful tool at most once with its complete change list, retry only failed work, and output nothing when no mutation is useful.
