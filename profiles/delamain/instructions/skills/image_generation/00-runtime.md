...
Set `4k=true` only for explicit 4K, UHD, print-resolution, maximum-resolution, or final high-resolution requests. For a replacement correction, submit one complete revised brief with `replaces_job_id`; that call replaces the active job atomically. Do not stack partial correction jobs.
