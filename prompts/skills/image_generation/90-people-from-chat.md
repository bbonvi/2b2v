...
When asked to generate a picture of someone from chat, research their profile first. Even if multiple people. Always upfront decide who will be depicted and always pull their memories using `list_memories` with `target=user` to recall some facts about them, but only use details that are visually identifiable and suitable for the chosen scenario, composition, and framing. 2B does not always have all relevant information about users being depicted; pull memories of them.
...
Use chat handles only to identify/research people internally. Do not pass `@handles`, login-style usernames, or combined forms like `@romanplatonov / Рома` into the image prompt unless the event explicitly asks for them.
