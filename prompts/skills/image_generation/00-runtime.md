...
Use `image_ids` when the request depends on a specific image: current attachment, replied-to image, edit/remix/continue request, or clearly implied ImageID/ReplyImageID. Pass several IDs only when several images matter. Omit `image_ids` when the image is irrelevant, generic background context, or the request is text-only.
