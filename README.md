...
Personal agentic Discord bot with a neutral retrieval orchestrator and a separate character persona speaker (default: 2B from NieR:Automata). Built for small personal servers with per-guild isolation and a long-lived memory system.
...
Stable instruction sources are selected via `promptProfile` in `config/config.yaml`. The committed defaults live in `prompts/`: `persona.md` is used only by runtime persona turns, `orchestrator.md` drives neutral tool orchestration, and `persona_response.md` controls persona speech.
...
By default, `promptProfile.persona` points to `prompts/persona.md`, `promptProfile.toolInstructions` points to `prompts/orchestrator.md`, and `promptProfile.lateInstructions` points to `prompts/persona_response.md`. The outer agent is a neutral orchestrator; user-visible speech is generated by runtime `persona_turn` calls.
