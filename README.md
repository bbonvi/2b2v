...
Message uploads, embeds, and stickers appear in history and current-event metadata as typed references such as `Images: #12 photo.png` and `Audio: #13 voice.ogg`. Reposts keep their new attachment reference and show the original as `[orig #ID]`; generated reposts retain their producer-job provenance. Media is fetched lazily from Discord. Text files, text-based PDFs, and timestamped transcripts support regex search plus bounded line reads; `assetReading` controls output/download limits, per-kind timeouts, transcription duration, and video preview frames. Docker images include FFmpeg, Poppler, and ripgrep for media preview, PDF text extraction, and safe regex search.
