...
`semanticMaintenance.burst` debounces the three private semantic passes per guild channel. `quietAfter` starts a burst after quiet activity, while `maxWait` caps delay during continuous activity. Durations use compact values such as `2m`, `4m`, or `6h`. `semanticMaintenance.sweep` controls infrequent bounded memory, relationship, and inner-thread corpus review. A new scope runs once when first triggered; its cadence and corpus cursors then survive restarts. Each path has one `modelProfile` for all three sequential passes. All settings support per-guild overrides.
