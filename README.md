...
`relationships` stores durable per-user relationship profiles: small axis scores, notes, boundaries, open loops, and recent relationship signals. A compact private slice is injected into normal replies when enabled, and a silent post-reply main-model pass may commit useful updates through `record_relationship`. It is not a life sim: no current activity, needs, heartbeat, plans, or proactive queue.
