...
Prompt assembly is file-based: `prompts/core/**/*.md` files are loaded into the stable prompt in deterministic recursive path order. Use numeric file or directory prefixes like `00-persona/`, `10-style/`, and `20-additional-instructions.md` when order matters. Runtime policy is loaded separately from `prompts/runtime/`: reply runtime from `reply/**`, final per-turn action text from `final-action-instruction/**`, memory context snippets from `memory/context/**`, tool descriptions from `tools/*.md`, parameter descriptions from `tool-parameters/<tool>/<param>.md`, volatile context templates from `context/*.md`, fallback vision instructions from `image-reading/fallback-system/**`, compact ambient evaluator policy from `ambient-attention/evaluator/{shared,ambient-pickup,lingering-attention,follow-up}/**`, and ambient initiative policy from `ambient-initiative/{evaluator,generation}/{shared,self-expression,targeted-checkin}/**`. On-demand skills live in `prompts/skills/<id>/skill.yaml`; normal turns receive only the compact skill index, while `load_skill` returns the manifest-ordered markdown body and tools listed in `required_for_tools` are blocked until their skill is loaded. Runtime templates support scalar `{{variable}}` placeholders; missing variables fail fast so prompt output stays deterministic. `promptTransport` can tune each provider's role/target placement for stable sections, volatile context sections, the current turn, and the final action instruction; defaults use Codex `split-input`, `developer` for behavior policy, and `user` for context/history/current-turn/final-action data. Ignored root files such as `prompts/persona.md` and `prompts/style.md` are legacy/live-local migration files; current code does not read them.

`memoryExtraction.postReply` controls the silent memory pass after visible bot replies. `memoryExtraction.maxToolCalls` caps record_memory calls per post-reply or ambient maintenance pass, defaulting to 5. `memoryExtraction.ambient` can periodically review non-triggered human chatter; `everyMessages` counts from the last successful memory pass in that channel, including post-reply passes, and `minIntervalSeconds` prevents bursty background spend.

`reasoningContinuation` is Codex-only. When enabled, the bot stores the latest encrypted provider-native continuation for the same guild/channel/user/model/session and replays it for nearby turns until `maxAgeMs` expires; it is not rendered into Discord history, search, or embeddings.

`ambientAttention` can create delayed non-trigger reply candidates: quiet-channel pickup, short-lived lingering attention after 2B replies to a user, and rare silence-gated follow-ups. Each mode owns its delays, thresholds, typing gate, cooldown, default reply anchoring, and hourly budgets; pickup and lingering attention debounce same-user message bursts into one pending evaluation. Lingering attention should usually have a shorter typing gate and higher budget because it models continuing a live exchange, not unsolicited pickup. Before evaluation and again before sending, the runtime reloads recent chat state and drops stale candidates when users are typing, the channel becomes busy, another human answers first, or newer context makes the candidate outdated. The evaluator model only returns a structured speak/silence decision; the normal persona reply loop writes the actual message.

`ambientInitiative` is proactive speech in the guild's main channel: self-expression and targeted follow-ups/check-ins. It uses jittered opportunity checks, active-hour and room-presence gates, per-kind pressure/caps/cooldowns plus shared fatigue, a small evaluator model for whether/kind/target/source/avoidances, and the normal reply model for final generation with `<ignore>` allowed. Prompt Lab can draft each initiative kind without sending to Discord and shows hard gates, pressure, evaluator output, generation, and pre-send vetoes in the dashboard.

`relationships` stores durable per-user relationship profiles: small axis scores, notes, boundaries, open loops, and recent relationship signals. A compact private slice is injected into normal replies when enabled, and a silent post-reply main-model pass may commit useful updates through `record_relationship`, capped by `relationships.maxToolCalls` with a default of 5. It is not a life sim: no current activity, needs, heartbeat, plans, or proactive queue.
...
Guild memories are shared server facts. User memories are keyed by Discord user ID and follow that user across guilds; memory text should avoid raw guild IDs and use natural local context only when it is essential. Self memories are the bot/persona's own portable relationships and private journal.
