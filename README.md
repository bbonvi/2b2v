...
Conversational dice requests use the shared `roll_dice` agent tool without a slash command or persistent panel. It uses cryptographic randomness, supports optional total-at-least target checks, records an idempotent local audit row, and posts the canonical result as a public Discord Components V2 card. That Discord message is the shared roll history visible to users and other bots in the channel; each profile's database remains isolated.
