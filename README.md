...
`contextHistory.retainedMessages` is the total channel history kept immediately after a rollover. `recentMessages` is both the minimum uncached recent tail and the rollover batch size. Thus, `600` retained and `100` recent keeps about 500 cached older messages, 100–199 recent messages, and 600–699 messages in total. Each additional 100 messages moves one recent batch into older history and removes one oldest batch when needed. `messageCharLimit` bounds older and auxiliary history rows; ordinary recent chat remains complete.
...
`semanticMaintenance.burst` debounces the three private semantic passes per guild channel. `quietAfter` starts a burst after quiet activity, while `maxWait` caps delay during continuous activity. Durations use compact values such as `2m`, `4m`, or `6h`. `semanticMaintenance.sweep` controls infrequent bounded memory, relationship, and inner-thread corpus review. A due sweep starts a minimal context with no conversation, semantic state, members, emojis, schedules, notebooks, agent jobs, repertoire, or parent-thread history. Its three passes share only the new sweep transcript. A new scope runs once when first triggered; its cadence and corpus cursors then survive restarts. Each path has one `modelProfile` for all three sequential passes. All settings support per-guild overrides.

Top-level `repertoire` periodically samples recent 2B exchanges from the rooms where she speaks most and injects them into other rooms as stable style references. It omits rooms with fewer than four usable excerpts, gives larger rooms proportionally more examples, and uses stable rotating slices of at most one quarter to limit repetition without durable state. If the configured lookback cannot safely supply the requested size, it retries once with twice that window. `lookbackHours`, `refreshMinutes`, `maxSourceChannels`, `maxMessages`, and `maxChars` bound selection and prompt size.
...
Async agent jobs are durable and visible to the persona across guilds, with their source and delivery locations retained. They include image generation and parallel background copies of the active persona. `spawn_agent` starts a self-contained long-running task; `send_agent_message` queues a running follow-up or resumes a waiting or yielded agent. Background agents reuse the persona prompt and tools without source-guild chat context, and their ordinary output returns as a private handoff. Their wall-clock and optional tool-call limits are configured under `agentJobs`; old rounds compact near the model context limit. Owned image jobs run concurrently, wake the parent on each result, and keep it active as `waiting_on_jobs` until no child remains. Completion handoffs re-enter the source channel dispatcher after queued user messages. `list_agent_jobs` and `read_agent_job` expose lifecycle, exact input, ownership, handoff, and output provenance. Yielded agents are dismissed after an hour without a follow-up once their handoff was processed; recent terminal jobs remain in prompt context for 15 minutes. Running work is requeued after restart.

`export_asset_to_workspace` copies a chat or staged asset into persistent workspace storage. `stage_workspace_file` exposes a workspace path as a staged asset for reading, image references, or Discord delivery. Generated images are written to the same staged volume automatically. Staged handles remain reusable after delivery. An hourly cleanup deletes staged files when their seven-day expiry time passes; moving a file outside the staged directory preserves it.
